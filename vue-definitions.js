...
let app = new Vue({
  ...
  methods: {
    ...
    processData(data) {
      ...
      const statesToLeaveOut = ["Diamond Princess", "Grand Princess"];
      ...
      for (let state of states) {
        ...
        if (!statesToLeaveOut.includes(state)) {
          ...
          myData.push({
            state,
            cases: arr.map(e => (e >= this.minCasesInState ? e : NaN)),
            slope: slope.map((e, i) =>
              arr[i] >= this.minCasesInState ? e : NaN
            )
          });
        }
      }
      ...
      this.covidData = myData.filter(
        e => this.myMax(...e.cases) >= this.minCasesInState
      );
